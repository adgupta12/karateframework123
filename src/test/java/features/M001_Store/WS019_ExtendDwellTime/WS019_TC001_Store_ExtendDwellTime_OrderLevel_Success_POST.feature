@All @WS019 @WS019_TC001 @Store @ExtendDwellTime
Feature: WS019_TC001_Store_ExtendDwellTime_OrderLevel_Success_POST

Background:
* print 'ws_url -> ', ws_url
* print 'currentDateTimeStamp -> ', currentDateTimeStamp

Scenario Outline: WS019_TC001_<scenarioId>_Store_ExtendDwellTime_OrderLevel_Success_POST
...
* def parcelArr = []
* set parcelArr[0] = parcelNumber1
* set parcelArr[1] = parcelNumber2
And match parcelArr contains $.response.payload.records[0].parcelId
...
And match parcelArr contains $.response.payload.records[1].parcelId
And match $.response.payload.records[1].status == 'Dwell time extended for Ready to collect parcels '
And match $.response.payload.records[1].extendedDate == '#regex [0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}'
